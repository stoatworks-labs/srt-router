# Running unsigned SRT Router builds
...
```sh
xattr -dr com.apple.quarantine ./srt-router
chmod +x ./srt-router
./srt-router --help
```
...
SRT Router listens on the network, so the first time it starts Windows shows:

> **Windows Defender Firewall has blocked some features of SRT Router**
...
domain. SRT Router needs this to serve the web UI on the interface you pick and accept inbound SRT streams.

If you deny it (or the prompt appears behind another window and times out), the UI won't load from another machine and inbound SRT callers will time out
...
**Private**. If SRT Router isn't listed, **Allow another app…** → **Browse** to the `.exe`.

Or from an elevated PowerShell:

```powershell
New-NetFirewallRule -DisplayName "SRT Router" -Direction Inbound -Program "C:\Path\To\SRT Router.exe" -Action Allow -Profile Private,Domain
```

To remove a bad rule and get the prompt back on next launch:

```powershell
Get-NetFirewallRule -DisplayName "*SRT Router*" | Remove-NetFirewallRule
```
...
```sh
codesign --force --deep --sign - "/Applications/SRT Router.app"
```

Verify it took:

```sh
codesign -dv --verbose=4 "/Applications/SRT Router.app"
spctl -a -vv "/Applications/SRT Router.app"   # still reports "rejected" — ad-hoc is not notarization
```
